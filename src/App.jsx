...
export default function App() {
  const {
    sessionsDoneToday, canDoSession, recordSession
  } = useBreathingStorage()
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center gap-6"
      ...
      <h1 className="text-lg font-bold text-slate-200">மூச்சுப் பயிற்சி</h1>

      {/* Session slots */}
      <div className="flex gap-3">
        {[0, 1].map(i => (
          <div key={i} className="px-4 py-2 rounded-lg text-sm"
            style={{ background: i < sessionsDoneToday ? '#38bdf822' : '#1e293b',
